...
<title><fmt:message key="login.requestAccess.title" /></title>
...
<body>
				...
				<div class="login100-form-title"
					...
					<p align="center">
						<span style="color: white"><fmt:message
								key="login.requestAccess.subtitle" /></span>
					</p>

				</div>


				<form:form method="POST" modelAttribute="userAccessForm"
					class="login100-form">

					<div class="wrap-input100 validate-input m-b-18">
						<span class="label-input100"><fmt:message
								key="login.requestAccess.email" /></span>
						<spring:bind path="email">
							<div
								class="form-control input100 ${status.error ? 'has-error' : ''}">
								<form:input type="text" path="email" class="input100"
									placeholder="Email" autofocus="true" maxlength="100"></form:input>
					...
					<div class="wrap-input100 m-b-18">
						<span class="label-input100"><fmt:message
								key="login.requestAccess.reason" /></span>
						<spring:bind path="reason">
							<div
								class="form-control input100 ${status.error ? 'has-error' : ''}">
								<form:textarea type="text" path="reason" class="input100"
									placeholder="" autofocus="true" rows="5" maxlength="150"></form:textarea>
						...
						<div class="form-check">
							<input type="checkbox" class="form-check-input" required
								...
								for="exampleCheck1"><fmt:message
									key="login.requestAccess.acceptTerms" /><a
								href="https://spilapps.unex.es/resources/contigo_privacy.pdf"
								target="_blank"><fmt:message
										key="login.requestAccess.acceptTermsText" /></a></label>
						</div>
					</div>
					...
					<button class="btn btn-lg btn-primary btn-block"
						...
						<fmt:message key="login.requestAccess.send" />
					</button>
					<input type="hidden" name="curLang" value="${lang}" />
				</form:form>
			</div>
		</div>
	</div>
	...
	<script
		src="https://www.google.com/recaptcha/api.js?hl=<fmt:message key='general.recaptcha' />"></script>
